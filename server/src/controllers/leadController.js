...
async function createLead(req, res) {
  ...
  try {
    ...
    if (!(await leadService.companyOwnedByUser(req.body.company_id, req.user))) {
      return res.status(403).json({ message: 'You cannot create a lead for this company' });
    }
...
async function updateLead(req, res) {
  ...
  try {
    ...
    if (req.body.company_id !== undefined) {
      ...
      if (!(await leadService.companyOwnedByUser(req.body.company_id, req.user))) {
        return res.status(403).json({ message: 'You cannot assign this lead to that company' });
      }
    }
