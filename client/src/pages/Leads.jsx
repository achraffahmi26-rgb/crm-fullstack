    ...
    <div className="space-y-5">
      <section className="flex flex-col justify-between gap-4 lg:flex-row lg:items-end">
        <div className="min-w-0">
          <p className="text-sm font-semibold uppercase tracking-wide text-crm-orange">Leads</p>
          <h1 className="mt-2 text-2xl font-semibold text-crm-ink md:text-3xl">Lead pipeline</h1>
      ...
      <section className="rounded-lg border border-crm-line bg-white shadow-sm">
        <div className="flex flex-col gap-3 border-b border-crm-line p-4 lg:flex-row lg:items-center lg:justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-base font-semibold text-crm-ink">All leads</h2>
        ...
        {isLoading ? (
          ...
          <div className="p-10 text-center">
            ...
            <h3 className="mt-4 text-base font-semibold text-crm-ink">No leads found</h3>
          ...
          <div className="overflow-x-auto">
            <table className="min-w-[1280px] w-full text-left">
              ...
              <tbody className="divide-y divide-crm-line text-sm">
                {table.rows.map((lead) => (
                  <tr className="bg-white hover:bg-crm-surface/70" key={lead.id}>
                    <td className="px-4 py-4">
                      <p className="font-semibold text-crm-ink">{getLeadName(lead)}</p>
                      <p className="text-xs text-crm-muted">ID #{lead.id}</p>
                    </td>
                    <td className="px-4 py-4 text-crm-muted">{lead.email || '-'}</td>
                    <td className="px-4 py-4 text-crm-muted">{lead.phone || '-'}</td>
                    <td className="px-4 py-4 text-crm-muted">{getCompanyLabel(lead.company_id)}</td>
                    <td className="px-4 py-4 text-crm-muted">{getUserLabel(lead.assigned_to)}</td>
                    <td className="px-4 py-4 text-crm-muted">{lead.source || '-'}</td>
                    <td className="px-4 py-4">
                      <StatusBadge status={lead.status} />
                    </td>
                    <td className="px-4 py-4 text-crm-muted">{formatMoney(lead.estimated_value)}</td>
                    <td className="px-4 py-4 text-crm-muted">{formatDate(lead.created_at)}</td>
                    <td className="px-4 py-4">
